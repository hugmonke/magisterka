...
def plot_parameter_space(X, Y, n_neighbors, min_dist, random_state):
    ...
    for state_type in color_map.keys():
        if state_type in Y.values:
            mask = Y == state_type
            plt.scatter(embedding[mask, 0]
                        , embedding[mask, 1]
                        , c=color_map[state_type]
                        , label=state_type
                        , alpha=0.7
                        , edgecolors='w'
                        , s=20
...
def train_rf_classifier(X, Y, features, n_estimators, test_size, random_state):
...
def main():
    ...
    X, Y = dataset[features], dataset['State']

    plot_parameter_space(X=X, Y=Y, n_neighbors=N_NEIGHBORS, min_dist=MIN_DIST, random_state=RANDOM_STATE)
    train_rf_classifier(X=X, Y=Y, features=features, n_estimators=N_ESTIMATORS, test_size=TEST_SIZE, random_state=RANDOM_STATE)
