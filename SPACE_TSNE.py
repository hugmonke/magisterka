...
def get_dataset(sim_num=1000, print_every=50, init_xyz=(0.1, 0.0, 0.0), params=None, dt=0.01, t_skip=50, t_end=150, size=1000, cutoff=1e6, batch_size=1000):
    ...
    with open("sim_results_space.txt", "a", encoding="utf-8") as file:
        ...
        while total_sim < sim_num:
            ...
            lle_all, valid_mask, x_all, y_all, z_all = com.solve_and_get_lle(
                init_xyz=init_xyz, params=params, dt=dt, t_skip=t_skip, 
                t_end=t_end, size=cur_size, cutoff=cutoff
            )
...
# t-SNE PLOTTING
# ------------------------------------------------------------
def plot_parameter_space_tsne(df_params, df_states, perplexity, random_state):
    """Scaling, t-SNE dimensionality reduction and interactive plotting."""
    print("=== Running t-SNE projection ===")
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(df_params)
    
    reducer = TSNE(n_components=2, perplexity=perplexity, random_state=random_state, init='pca', learning_rate='auto')
    embedding = reducer.fit_transform(X_scaled)
    
    # PARAMETER SPACE PLOT (t-SNE Projection)
    fig, ax = plt.subplots(figsize=(12, 8))
    color_map = {
        "CHAOTIC": "red", "STABLE": "green", "PERIODIC": "blue",
    ...
    plt.title("Parameter Space Mapping (t-SNE)")
...
def main():
    ...
    dataset = get_dataset(
        sim_num=SIM_NUM, print_every=PRINT_EVERY, init_xyz=INIT_XYZ, params=PARAMS, 
        dt=DT, t_skip=T_SKIP, t_end=T_END, size=SIM_NUM, cutoff=CUTOFF, batch_size=BATCH_SIZE
    )
    ...
    plot_parameter_space_tsne(df_params=df_params, df_states=df_states, perplexity=PERPLEXITY, random_state=RANDOM_STATE)
